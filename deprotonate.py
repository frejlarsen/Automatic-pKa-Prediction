...
if __name__ == "__main__":
    ...
    if len(bonded_to_N) == 2:
        ...
        bonded0 = bonded_to_N[0]
        bonded1 = bonded_to_N[1]              

        for x in range(len(bonded0)):
            for y in range(len(bonded1)):
                MolNumber += 1
                Namestring = filename[:-4] + str(MolNumber)
                Namestring = Namestring.replace("-", "_") + "-.xyz"
                ...
                f.write('{0:3d}'.format(atoms-2))
                f.write('{0:50s}'.format(title))
		f.write("\n")

                
                for i in range(atoms):
                    if i == bonded0[x] or i == bonded1[y]:
                        continue
                    else:
                        Writestring = "\n" + str(atom[i]) + " " + str(coord[i][0]) + " " + str(coord[i][1]) + " " + str(coord[i][2])
                        f.write(Writestring)
    ...
    # MOLECULE HAS 3 NITROGEN ATOM
                        
    if len(bonded_to_N) == 3:

        bonded0 = bonded_to_N[0]
        bonded1 = bonded_to_N[1]              
        bonded2 = bonded_to_N[2]

        for x in range(len(bonded0)):
            for y in range(len(bonded1)):
                for z in range(len(bonded2)):
                    ...
                    f.write('{0:3d}'.format(atoms-3))
                    f.write('{0:50s}'.format(title))
                    f.write("\n")

                    for i in range(atoms):
                        if i == bonded0[x] or i == bonded1[y] or i == bonded2[z]:
                            continue
                        else:
                            Writestring = "\n" + str(atom[i]) + " " + str(coord[i][0]) + " " + str(coord[i][1]) + " " + str(coord[i][2])
                            f.write(Writestring)
    ...
    # MOLECULE HAS 4 NITROGEN ATOM    
    
    if len(bonded_to_N) == 4:
    
        bonded0 = bonded_to_N[0]
        bonded1 = bonded_to_N[1]              
        bonded2 = bonded_to_N[2]    
        bonded3 = bonded_to_N[3]
        
        for x in range(len(bonded0)):
            for y in range(len(bonded1)):
                for z in range(len(bonded2)):
                    for w in range(len(bonded3)):                        
                        MolNumber += 1
                        Namestring = filename[:-4] + str(MolNumber)
                        Namestring = Namestring.replace("-", "_") + "-.xyz"
                        ...
                        f.write('{0:3d}'.format(atoms-4))
                        f.write('{0:50s}'.format(title))
			f.write("\n")
                
                        for i in range(atoms):
                            if i == bonded0[x] or i == bonded1[y] or i == bonded2[z] or i == bonded3[w]:
                                continue
                            else:
                                Writestring = "\n" + str(atom[i]) + " " + str(coord[i][0]) + " " + str(coord[i][1]) + " " + str(coord[i][2])
                                f.write(Writestring)
    ...
    if len(bonded_to_N) == 0 or len(bonded_to_N) > 4:
        print "0 or more than 4 nitrogen atoms were present. Please complain @ Frej."
